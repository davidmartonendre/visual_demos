...
const fragmentShader = `
  ...
  void main() {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      plasma += sin(x + time * 0.4 + fi * 1.2 + mx * 0.4);
      plasma += sin(y - time * 0.35 + fi * 1.8 + my * 0.4);
      plasma += sin((x + y) * 0.7 + time * 0.6 + fi * 2.5);
      plasma += sin(length(p - mp * 0.3) * (4.0 + fi * 2.5) - time * (0.8 + fi * 0.4));
      plasma += sin(atan(p.y - mp.y, p.x - mp.x) * 2.0 + fi * 3.14 - time * 0.5);
    }

    plasma /= 25.0;
    plasma = plasma * 0.5 + 0.5;

    float contrast = pow(plasma, 1.5);
    float highlight = pow(plasma, 4.0) * 0.5;

    float clickWave = 0.0;
    if (clicked > 0.0) {
      vec2 cp = (clickPos - 0.5 * resolution) / min(resolution.x, resolution.y);
      float cd = length(p - cp);
      clickWave = sin(cd * 25.0 - time * 8.0) * exp(-cd * 4.0 - time * 0.3);
    }

    float pattern = contrast + highlight + clickWave * 0.3;
